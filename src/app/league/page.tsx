      ...
      <div className="rounded-xl border border-white/5 bg-slate-900/60 p-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <input
            ...
            inputMode="numeric"
            className="min-w-[220px] flex-1 rounded-lg border border-white/10 bg-slate-950/60 px-3 py-3 text-sm text-slate-100 placeholder:text-slate-400 focus:border-volt/50 focus:outline-none"
          />
          <button
            type="button"
            onClick={loadLeague}
        ...
        {platform === "ESPN" && (
          <div className="mt-4 space-y-3 border-t border-white/5 pt-4">
            ...
            <div>
              <label htmlFor="fft-espn-s2" className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wider text-slate-400">
              ...
              <textarea
                ...
                rows={2}
                spellCheck={false}
                className="w-full resize-y rounded-lg border border-white/10 bg-slate-950/60 px-3 py-2.5 font-mono text-xs text-slate-100 placeholder:text-slate-400 focus:border-volt/50 focus:outline-none"
              />
            </div>
            <div>
              <label htmlFor="fft-espn-swid" className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wider text-slate-400">
                SWID cookie
              </label>
              <input
                id="fft-espn-swid"
                ...
                spellCheck={false}
                className="w-full rounded-lg border border-white/10 bg-slate-950/60 px-3 py-2.5 font-mono text-xs text-slate-100 placeholder:text-slate-400 focus:border-volt/50 focus:outline-none"
              />
            </div>
          </div>
        )}
                ...
                <tbody>
                  {data.teams.map((team, index) => (
                    <Fragment key={team.rosterId}>
                      ...
                      >
                        <td className="py-3 pl-4 pr-2 text-slate-400">{index + 1}</td>
                        <td className="px-2 py-3">
                          <p className="font-medium text-slate-100">{team.teamName}</p>
                          {team.displayName && team.displayName !== team.teamName && (
                            <p className="text-xs text-slate-400">{team.displayName}</p>
                          )}
                        </td>
          ...
          {tab === "trades" && (
            <div className="space-y-4">
              ...
              {trades && trades.length === 0 && !tradesLoading && (
                <div className="rounded-xl border border-white/5 bg-slate-900/60 px-5 py-10 text-center text-sm text-slate-400">
          ...
          {tab === "power" && (
            <div className="space-y-2">
              {power.map((entry) => (
                ...
                >
                  <span className="w-7 text-center font-display text-2xl text-slate-400">
                    {entry.rank}
                  </span>
              ...
              {stored?.leagueId === data.league.id && (
                <>
                  <div>
                    ...
                    {proposals.length === 0 ? (
                      <p className="rounded-xl border border-white/5 bg-slate-900/60 px-5 py-8 text-center text-sm text-slate-400">
                        ...
                        {proposals.map((proposal, index) => (
                          ...
                          >
                            <div className="flex flex-wrap items-center gap-3">
                              ...
                              <span className="font-display text-xl text-slate-400">for</span>
    ...
    <div className="mb-4 rounded-lg border border-sky-500/20 bg-sky-500/5 p-4">
      ...
      <div className="flex flex-wrap gap-1.5">
        {lineup.starters.map((slot, index) => (
          ...
          >
            <span className="text-slate-400">{slot.slot}</span>{" "}
